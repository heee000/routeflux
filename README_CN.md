...
它是原始 DomainRouter 项目的平台化延续：把 12 领域、14 维可解释路由放入多用户 API 钱包，并加入受 R2-Router 启发的模型与输出预算联合优化。研究来源见[研究脉络](docs/research/README.md)。
...
- 在手动或自动路由之前执行模型白名单限制
- 跨多个网关实例的 API Key 预算原子预占
- 自动回收过期的钱包预占
- 严格按已授权额度结算，并审计 Provider 实际成本缺口
- 可正确处理 UTF-8 分片的流式用量计量
- 对高于预测输出长度的 Token 预算采用边际递减质量增益
- 恢复原始 DomainRouter 的 12 领域中英双语 TF-IDF n-gram 分类
- 持久化 14 维难度明细和四档难度等级
- 兼容 v0.7 的宽领域模型画像
- 保存研究来源与本轮对话交接文档
...
`allowed_models` 为空数组时，Key 可以调用模型目录中的全部启用模型。限流响应会返回剩余次数和重置时间；预算检查与钱包预占位于同一个 PostgreSQL 事务中，并发请求不会分别花掉同一份 Key 预算。
