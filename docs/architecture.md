...
RouteFlux separates the request path into four parts:

1. The HTTP gateway validates OpenAI-compatible requests and authenticates API keys.
2. The router filters models by hard capabilities, scores eligible model and token-budget pairs, and returns an ordered decision.
3. The provider adapter translates the selected catalog entry into an upstream request and forwards normal or streaming responses.
4. The accounting path reserves the maximum request charge, records observed usage, settles the hold, and appends an immutable ledger entry.
